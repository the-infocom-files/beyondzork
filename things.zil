...
<ROUTINE TO-SPLENDOR ("AUX" DIR X Y)
...
<ROUTINE ARCH-F ("OPT" (CONTEXT <>) "AUX" OBJ X)
	 <COND (<T? .CONTEXT>
		<COND (<EQUAL? .CONTEXT ,M-OBJDESC>
...
<ROUTINE GENERIC-DIAL-F (TBL "AUX" X Y)
	 <SET X <GET .TBL 1>>
	 <SET Y <GET .TBL 2>>
	 <COND (<EQUAL? .X ,GURDY>
		<RETURN .Y>)
...
<ROUTINE ROLL-ONION (DIR "AUX" OHERE)
	 <COND (<SPARK? <> ,ONION>
		<TELL ,TAB>)>
...
<ROUTINE MUNG-STALL ("OPT" (OBJ ,PRSI))
	 <ITALICIZE "Wham">
	 <TELL "! ">
	 <YOUR-OBJ .OBJ>
...
<ROUTINE TRUFFLE-F ("AUX" (FRESH 0) WRD)
...
<ROUTINE DIAMOND-F ("OPT" (CONTEXT <>) "AUX" TBL)
	 <COND (<T? .CONTEXT>
		<COND (<EQUAL? .CONTEXT ,M-OBJDESC>
